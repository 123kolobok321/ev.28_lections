...
#     f. double precsion (8 bytes) -> real, но только с двойной точностью (12 цифр после запятой)
...
# ORDER BY: позволяет нам сортировать входящие данные по убыванию или возрастанию. SAC(по возрастанию) и DESC (по убыванию)
...
# LIKE: выводит результат который соответствует введённому шаблону для строк Чувствителен к регистру
...
# GROUP BY: разделяет данные которые мы получаем в SELECT, при этом групируя их по определенному признаку. И теперь для каждой группы можно использовать функцию
...
# связи между таблицами(relations):
        # 1. Один к одному(one to one)- человек и паспорт
            # в одну из таблиц добовляется поле fk и даётся ограничение unique
        # 2. Один ко многим(one to many)- человек и банковские карты
            # в таблицу много (баковские карты) доп. поле fk
        # 3. много ко многим(many to many)- студенты и преподователи
            # создаётся вспомогательная 3 ья таблица со связями
# Ограничения:
    # 1. NOT NULL - обязательно к заполнению
    # 2. UNIQUE - то что будет храниться только уникальные данные
    # 3. CHECK -> CHECK age > 0 - ограничение проверки на условие
    # 4. PRIMARY KEY(для установки индефикатора данных в таблице)
...
# JOIN: выборка данных из двух таблиц, соендинение табли
...
# SELECT p1.title, p1.price, o1.quantity, p1.price * o1.quantity as total_sum FROM products p1 JOIN orders o1 ON p1.id = o1.prduct_id
