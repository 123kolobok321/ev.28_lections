# class methods, instance methods, static method

# instance methods - обычные методы, которые принемают первым аргументом self(ссылка на обьект). Нужны они чтобы внутри метода мы могли работать с атрибутами обьекта, получать их или изменять
...
# obj.instance_method(5) # если вызывать у обьекта, то в self передаются автоматически
# Test.instance_method(obj, 5) # если вызывать у класса, то в self нужно передать обьект вручную


# class methods - методы, которые принимают первым аргументом cls(ссылка на класс). Ныжны они для создания или изменения аттрибутов класса и для манипуляций с классом внутри метода. Создается при помощи декоратора @classmethod
...
# a = datetime.now()
# print(a.year, type(a))
# print(a.strftime('%H:%M:%S'), type(a.year))


# staticmethod - это те методы в классе которые не принемают в качестве аргументов ни  класс ни обьект, так называемые методы которые не изменяют состояние
