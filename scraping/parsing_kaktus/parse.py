...
def get_articles(catalog):
    ...
    # print(len(articles[0: 25]))
    return articles


def get_detail(link):
    html = get_html('https://kaktus.media/doc/479629_kinyl_moshennikov._v_moskve_taksist_jenishbek_vernyl_polmilliona_84_letney_jenshine.html')
    soup = get_soup(html)
...
def get_data(articles):
    ...
    for item in articles:
        ...
        discription = get_detail(link)
        data = {'title': title, 'img': image, 'desc': discription}
        res[i] = data
        i += 1
    return res
